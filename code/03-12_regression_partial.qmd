...
ggplot(reg_dfw, 
       aes(x = sr_ksha, 
           y = yield_mgha)) + geom_point() + geom_smooth()
...
# Model fitting
mod1_int = lmer(yield_mgha ~ (1|rep), data = reg_dfw)
...
nd = nd %>%
  mutate(mod1_yield_mgha = predict(mod1_int, nd, re.form = NA))
...
ggplot(mod2_lin_aug, 
       aes(x = sr_ksha, 
           y = yield_mgha))+
  geom_point(size = 3, 
             alpha = .7) + geom_line(aes(y = .fixed), color = "blue")
```
...
#Extra Bootstrapping 
Goal of bootstrapping is estimating confidence intervals.
...
ggplot(data = boot_mean,
  ...
  geom_vline(data = boot_ci,
             aes(xintercept = value), linetype = 2, color = "red")
